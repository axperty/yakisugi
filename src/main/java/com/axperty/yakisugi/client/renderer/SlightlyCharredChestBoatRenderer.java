...
public class SlightlyCharredChestBoatRenderer extends EntityRenderer<SlightlyCharredChestBoatEntity> {
    private final ChestBoatModel boatModel;
    private final ResourceLocation texture;

    public SlightlyCharredChestBoatRenderer(EntityRendererProvider.Context context) {
        super(context);
        ModelLayerLocation modellayerlocation = ModelLayers.createChestBoatModelName(Boat.Type.OAK);
        this.boatModel = new ChestBoatModel(context.bakeLayer(modellayerlocation));
        this.texture = ResourceLocation.fromNamespaceAndPath(Yakisugi.MOD_ID, "textures/entity/chest_boat/slightly_charred.png");
    }

    @Override
    public void render(SlightlyCharredChestBoatEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        poseStack.pushPose();
    ...
    @Override
    public ResourceLocation getTextureLocation(SlightlyCharredChestBoatEntity entity) {
        return this.texture;
    }
}
